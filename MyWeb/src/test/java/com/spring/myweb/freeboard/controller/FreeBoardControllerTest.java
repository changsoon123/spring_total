...
//	private FreeBoardController controller;
	...
	public void setup() {
		//가상 구조를 세팅
		//스프링 컨테이너에 등록된 모든 빈과 의존성 주입까지 로드에서 사용이 가능.
...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	@Test
	@DisplayName("6번 글 상세 보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글"
			+ "객체를 model에 담아서 jsp로 이동시킬 것이다.")
	...
	void testContent() throws Exception {
		...
		System.out.println("Model 내에 저장한 데이터: " + mv.getModelMap());
		System.out.println("응답 처리를 위한 페이지: " + mv.getViewName());
	}
	
	@Test
	@DisplayName("6번 글의 제목과 내용을 수정하는 요청을 post방식으로 전송하면 수정이 진행 되고,"
...
//		String bno = "6";
		...
		String viewpage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")
				.param("bno", "6")
				.param("title", "1 이게 될까???")
				.param("content", "1 제발 되어라"))				
				.andReturn()
				.getModelAndView().getViewName();
		
		assertEquals("redirect:/freeboard/content?bno=6", viewpage);
	...
	@Test
	@DisplayName("6번 글을 삭제하면 목록 재 요청이 발생할 것이다.")
	// /freeboard/delete -> post
	void testDelete() throws Exception {
		assertEquals("redirect:/freeboard/list",
