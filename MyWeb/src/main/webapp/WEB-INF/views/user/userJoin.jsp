...
<script>
        ...
        document.getElementById('idCheckBtn').onclick = function() {
            ...
            fetch API: 자바스크립트에서 제공하는 비동기 통신 함수.
          	...
          	소비할 수 있게 해 준다. (Promise: 비동기 통신의 순서를 보장하는 문법)
        ...
        document.querySelector('.mail-check-input').onblur = function(e){
        	...
        	if(inputCode === code){
        		...
        		e.target.style.display = 'none'; //인증 번호 입력창 숨기기
        		
        		//초기값을 사용자가 선택한 값으로 무조건 설정하는 방법(select에서 readonly 대용)
        		//항상 2개 같이 써야 한다.
        		const email2 = document.getElementById('userEmail2');
        		
        		email2.setAttribute('onFocus', 'this.initalSelect = this.selectedIndex');
        ...
        id.onkeyup = function() {
            /*자바스크립트의 정규표현식 입니다*/
            /* 정규 표현식: 문자열 내의 특정 문자 조합을 찾기 위한 패턴이다.
