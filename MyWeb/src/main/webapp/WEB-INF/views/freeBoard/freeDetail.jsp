...
<script>
    ...
    window.onload = function() {
        ...
        document.getElementById('moreList').onclick = () => {
        	/*
        	 왜 false를 줄까?
        	 더보기 라서 , 댓글을 누적해서 보여줘야 된다.
        	 1페이지의 댓글 내용 밑에다가 2페이지를 누적해서 깔아야 한다.
        	 1페이지 내용을 없애고 2페이지를 보여주는게 아니기 때문이다.
        	*/
        	getList(++page,false);
        }
        ...
        getList(1, true); //게시글 상세보기 화면에 처음 진입했을 시 댓글 리스트를 불러오기.
            ...
            fetch('${pageContext.request.contextPath}/reply/getList/' + bno + '/' + pageNum)
            	.then(res => res.json())
            		.then(data => {
            			...
            			//페이지 번호 * 이번 요청으로 받은 댓글 수보다 전체 댓글 개수가 작다면
            			//더보기 버튼은 없어도 된다.
        ...
        보장이 없기 때문에 해당 방식은 이벤트 등록이 불가능하다.
        이 때는, 이미 실제로 존재하는 #replyLis에 등록하고, 이벤트를 자식에게 위임하여
        사용하는 addEventListener를 통해 처리를 해야한다.
        ...
        */
        ...
        document.getElementById('replyList').addEventListener('click', e => {
        	...
        	if(e.target.classList.contains('replyModify')){
        		...
        		//제이 쿼리를 이용해서 bootstrap 모달을 여는 방법
		...
		document.getElementById('modalDelBtn').onclick = () => {
			/*
				...
				삭제 완료되면 모달 닫고 목록 요청 다시 보냄.(reset의 여부를 잘 판단)
			*/
			const reply = document.getElementById('modalReply').value;
			const rno = document.getElementById('modalRno').value;
			const replyPw = document.getElementById('modalPw').value;
