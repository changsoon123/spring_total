	...
	<script>
		...
		function regist(){
			...
			if(file !== 'jpg' && file !== 'png' && file !== 'jpeg' && file !== 'bmp'){
					...
					document.getElementById('file').vlaue = '';
					return;
			...
			//FormData 객체를 활용한다.
			//무조건 이 방식이 옳은 것은 아니다. FormData는 비동기 방식이 꼭 필요한 경우에만 사용한다.
			//대부분의 경우에는 form 태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용된다.
			
			const formData = new FormData();
			const $data = document.getElementById('file'); //이미지 첨부
			...
			//우리는 지금 multiple이 없기 때문에 [0]이라고 작성하면 파일 정보를 얻을 수 있다.
			
			//FormData 객체에 사용자가 업로드한 파일의 정보가 들어있는 객체를 전달.
			formData.append('file',$data.files[0]); //file은 파라미터 변수명
		...
		document.getElementById('contentDiv').addEventListener('click', e => {
			...
			//전달 받은 글 내용을 미리 준비한 모달창에 뿌릴 겁니다.(모달 위에 있습니다.)
			//값을 제 위치에 배치하시고 모달을 열어 주세요.
			//(부트 스트랩 모달이기 때문에 jQuery로 열어주세요.)
		...
		*/
		...
		서비스하는 형식에 맞는 페이징 방식을 적용하면 된다.
		
		*/
		window.onscroll = function(){
			if(!isFinish){
				/*
				윈도우(디바이스)의 높이와 현재 스크롤 위치 값을 더한 뒤,
				...
				*/
				...
				if(window.innerHeight + window.scrollY >= document.body.scrollHeight){
					//사용자의 스크롤이 바닥이 닿았을 때, 페이지 변수의 값을 하나 올리고
					//reset여부는 false를 주고 누적해서 계속 불러오면 된다.
